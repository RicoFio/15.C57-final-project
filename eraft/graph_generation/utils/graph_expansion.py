...
def add_edges_to_graph(g: ig.Graph, osm_graph: nx.MultiDiGraph,
                       ...
                       max_distance: float = None, max_travel_time: float = None,
    ...
    # Remove the edges that are self-loops
    ...
    if max_distance is not None:
        valid_edges = (0 < distances) & (distances <= max_distance)
    else:
        valid_edges = 0 < distances

    if max_travel_time is not None:
        travel_times = (distances / (speed*1000)) * 60
        valid_edges &= (0 < travel_times) & (travel_times <= max_travel_time)
