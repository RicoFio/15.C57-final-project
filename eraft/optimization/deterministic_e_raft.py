...
def deterministic_e_raft(g: Graph, planning_budget: int,
    ...
    for s, scenario in enumerate(scenarios):
        scenario_model = base_e_raft(
            ...
            use_single_scenario=s, fixed_adaptation_decisions=adaptations_ev
        )

        scenario_obj_value = scenario_model.getObjective().getValue()
        logger.info(f"Value for scenario {s}: {scenario_obj_value}")
        expected_objective += scenario.probability * scenario_obj_value

    return expected_objective, adaptations_ev
