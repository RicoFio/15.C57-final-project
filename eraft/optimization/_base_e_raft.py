...
def base_e_raft(g: Graph, planning_budget: int,
                      ...
                      fixed_adaptation_decisions: dict = None) -> Model:
    ...
    for i, s in enumerate(scenarios):
        for p in od_pairs:
            for a in edges:
                ...
                w_vars[s, p, a] = model.addVar(name=f'w_{i}_{p}_{a}', vtype=GRB.BINARY)
                # Variable introduced to represent the non-linearity
                q_vars[s, p, a] = model.addVar(name=f'q_{i}_{p}_{a}', vtype=GRB.BINARY)
    ...
    for i, s in enumerate(scenarios):
        for a in edges:
            z_vars[s, a] = model.addVar(name=f'z_{i}_{a}', vtype=GRB.BINARY)
    ...
    model.setObjective(
        quicksum(s.probability *
            quicksum(
                path_weighting[p] * (edges[a] * w_vars[s, p, a] + s.get_tt_impact(a) * q_vars[s, p, a])
    ...
    for s in scenarios:
        ...
        model.addConstrs((s.get_severity(a) - x_vars[a] <= big_m * z_vars[s, a] for a in edges), 'big_m_trick_UB')
        model.addConstrs((s.get_severity(a) - x_vars[a] >= -big_m * (1 - z_vars[s, a]) for a in edges), 'big_m_trick_LB')
    ...
    if model.status == GRB.OPTIMAL:
        ...
        for i, s in enumerate(scenarios):
            ...
            for a in edges:
                logger.info(f"\t Adaptation success {s.get_severity(a) - x_vars[a].X} "
        ...
        for p in od_pairs:
            ...
            for a in edges:
                exp_tt = sum([s.probability * (edges[a] + s.get_tt_impact(a) * w_vars[s, p, a]) for s in scenarios])
